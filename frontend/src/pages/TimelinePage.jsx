...
// Chapter dividers between story phases
const chapters = {
  0: { label: 'Chapter One', title: 'The Meeting' },
  4: { label: 'Chapter Two', title: 'Falling in Love' },
  7: { label: 'Chapter Three', title: 'Adventures Together' },
  10: { label: 'Chapter Four', title: 'A New Beginning' },
};

const ChapterDivider = ({ label, title }) => (
  <div className="relative my-20 flex flex-col items-center">
    <div className="flex items-center gap-4 mb-3">
      <div className="w-16 h-[1px] bg-gradient-to-r from-transparent to-[#b8956b]/50" />
      <span className="text-[#b8956b] text-xs tracking-[0.3em] uppercase font-medium">{label}</span>
      <div className="w-16 h-[1px] bg-gradient-to-l from-transparent to-[#b8956b]/50" />
    </div>
    <h2 className="font-cormorant text-2xl md:text-3xl text-[#3d3d38] italic">{title}</h2>
  </div>
);
        ...
        <div className="relative">
          ...
          {timeline.map((item, index) => {
            ...
            return (
              <div key={index}>
                {/* Chapter Divider */}
                {chapter && <ChapterDivider label={chapter.label} title={chapter.title} />}
                        ...
                        <div className={`flex flex-wrap justify-center gap-4 ${item.photos.length >= 3 ? 'flex-row items-start' : ''}`}>
                          {item.photos.map((photo, photoIndex) => {
                            ...
                            return (
                              <div 
                                key={photoIndex} 
                                className="group cursor-pointer transition-all duration-500 hover:z-10"
                                style={{
                                  transform: `rotate(${rotation}deg)`,
                                }}
                              >
                                {/* Polaroid Frame with lift + glow on hover */}
                                <div 
                                  className={`bg-white ${isCompact ? 'p-1.5 pb-8' : 'p-2 pb-12'} transition-all duration-500 relative group-hover:-translate-y-2`}
                                  style={{
                                ...
                                >
                                  ...
                                  <div 
                                    ...
                                    style={{
                                      boxShadow: '0 8px 30px rgba(184, 149, 107, 0.35), 0 20px 50px rgba(184, 149, 107, 0.15)',
                                    }}
                                  />
